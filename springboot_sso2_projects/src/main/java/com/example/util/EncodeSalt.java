...
public class EncodeSalt {
    ...
    public static  String getFinalSalt(){
        // 生成一个16位的随机数
    ...
    public static String getSaltMD5(String password,String hashAlgorithm,int number) {
        String finalSalt = getFinalSalt();
        // 多次盐值加密
        while(number>0){
            password = toStringHex(password + finalSalt,hashAlgorithm);
            number--;
        }
        char[] cs = new char[48];
        for (int i = 0; i < 48; i += 3) {
            cs[i] = password.charAt(i / 3 * 2);
            char c = finalSalt.charAt(i / 3);
            cs[i + 1] = c;
    ...
    public static void main(String[] args) {
        String md5 = EncodeSalt.getSaltMD5("123456", "md5", 2019);
        String finalSalt = EncodeSalt.getFinalSalt();
        System.out.println(md5);
        System.out.println(finalSalt);
    }
  }
